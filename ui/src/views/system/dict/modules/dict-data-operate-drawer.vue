...
async function handleSubmit() {
  ...
  // request
  if (props.operateType === 'add') {
    const { dictSort, dictLabel, dictValue, dictType, cssClass, listClass, isDefault, remark, attr } = model;
  ...
  if (props.operateType === 'edit') {
    const { dictCode, dictSort, dictLabel, dictValue, dictType, cssClass, listClass, isDefault, remark, attr } = model;
  ...
  <NDrawer v-model:show="visible" :title="title" display-directive="show" :width="800" class="max-w-90%">
    <NDrawerContent :title="title" :native-scrollbar="false" closable>
      <NForm ref="formRef" :model="model" :rules="rules">
        ...
        <NFormItem :label="$t('page.system.dict.data.attr')" path="attr">
          <NInput
            v-model:value="model.attr"
            type="textarea"
            :rows="4"
            :placeholder="$t('page.system.dict.form.attr.required')"
          />
        </NFormItem>
